...
async function assignTeamsToGroups(tournamentId, assignments) {
    ...
    try {
        ...
        await client.query(`DELETE FROM matches WHERE tournament_id = $1 AND stage = 'groups' 
        ...
        for (const { teamId, groupName } of assignments) {
            await client.query(`INSERT INTO tournament_standings (tournament_id, team_id, group_name)
                 VALUES ($1, $2, $3)
                 ON CONFLICT (tournament_id, team_id) DO UPDATE SET group_name = $3`, [tournamentId, teamId, groupName]);
        }
...
async function getGroupsWithTeams(tournamentId) {
    const client = await database_1.db.connect();
    try {
        const { rows: assignedRows } = await client.query(`SELECT ts.group_name, ts.team_id, t.name as team_name, t.tag as team_tag, 
                    t.logo_url as team_logo, ts.wins, ts.losses, ts.points
             FROM tournament_standings ts
             JOIN teams t ON ts.team_id = t.id
             WHERE ts.tournament_id = $1 AND ts.group_name IS NOT NULL
             ORDER BY ts.group_name, t.name`, [tournamentId]);
        const { rows: registeredRows } = await client.query(`SELECT t.id as team_id, t.name as team_name, t.tag as team_tag, t.logo_url as team_logo
             FROM tournament_registrations tr
             JOIN teams t ON tr.team_id = t.id
             WHERE tr.tournament_id = $1 AND (tr.status = 'APPROVED' OR tr.status = 'approved')`, [tournamentId]);
...
async function createCustomStage(tournamentId, stageData) {
    const { rows } = await database_1.db.query(`INSERT INTO stages (tournament_id, name, type, order_index, config)
         VALUES ($1, $2, $3, $4, $5) RETURNING *`, [tournamentId, stageData.name, stageData.type, stageData.orderIndex || 0, stageData.config || {}]);
    return rows[0];
}
async function generateGroupStageMatches(tournamentId, bestOf = 1) {
    ...
    try {
        ...
        const { rows: standings } = await client.query(`SELECT team_id, group_name FROM tournament_standings 
             WHERE tournament_id = $1 AND group_name IS NOT NULL ORDER BY group_name, team_id`, [tournamentId]);
        ...
        for (const [groupName, teamIds] of Object.entries(groups)) {
            ...
            for (let round = 0; round < numRounds; round++) {
                for (let match = 0; match < matchesPerRound; match++) {
                    ...
                    if (home === -1 || away === -1)
                        continue;
                    await client.query(`INSERT INTO matches (tournament_id, stage, group_name, round, team_a_id, team_b_id, best_of, status)
                         VALUES ($1, 'groups', $2, $3, $4, $5, $6, 'scheduled')`, [tournamentId, groupName, round + 1, home, away, bestOf]);
                    totalMatches++;
                }
                const fixed = teams[0];
