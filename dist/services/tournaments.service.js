...
async function deleteTournament(id) {
    const result = await database_1.db.query(`DELETE FROM tournaments WHERE id = $1 RETURNING *;`, [id]);
    return result.rows[0];
}
...
async function generateGroupStage(tournamentId, numberOfGroups = 1, bestOf = 1) {
    ...
    try {
        ...
        const { rows: teams } = await client.query(`SELECT team_id FROM tournament_registrations WHERE tournament_id = $1 AND status IN ('approved', 'APPROVED')`, [tournamentId]);
        if (teams.length < 2)
        ...
        for (let g = 0; g < groups.length; g++) {
            ...
            for (let i = 0; i < groupTeams.length; i++) {
                for (let j = i + 1; j < groupTeams.length; j++) {
                    await client.query(`INSERT INTO matches (tournament_id, stage, group_name, team_a_id, team_b_id, best_of, status)
             VALUES ($1, 'groups', $2, $3, $4, $5, 'scheduled')`, [tournamentId, groupName, groupTeams[i].team_id, groupTeams[j].team_id, bestOf]);
                }
            }
        }
        await client.query('COMMIT');
    ...
    try {
        ...
        while (currentMatches > 1 || (round === 2 && currentMatches === 1)) {
            ...
            if (nextCount > 0) {
             ...
             VALUES ($1, 'playoffs', $2, $3, $4, 'scheduled')`, [tournamentId, round, i, bestOf]);
                }
            }
            else
                break;
