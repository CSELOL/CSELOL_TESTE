...
const createTeam = async (name, tag, logoUrl, description, socialMedia, userId) => {
    ...
    try {
        ...
        const teamResult = await client.query(`INSERT INTO teams (name, tag, logo_url, description, social_media, captain_id, invite_code)
       VALUES ($1, $2, $3, $4, $5, $6, $7)
       RETURNING *`, [name, tag, logoUrl, description, socialMedia, userId, inviteCode]);
        const team = teamResult.rows[0];
        await client.query(`UPDATE users SET team_id = $1, team_role = 'CAPTAIN' WHERE supabase_id = $2`, [team.id, userId]);
...
const joinTeam = async (userId, teamId) => {
    const result = await database_1.db.query(`UPDATE users SET team_id = $1, team_role = 'MEMBER' WHERE id = $2 RETURNING *`, [teamId, userId]);
...
const getTeamTournaments = async (teamId) => {
    const result = await database_1.db.query(`SELECT t.id, t.tournament_name as name, t.status,
